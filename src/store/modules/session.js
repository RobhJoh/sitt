...
const mutations = {
  ...
  addHistory(state, players) {
    ...
    state.voteHistory.push({
      ...
      type: isExile ? 'Landsförvisning' : 'Utkastning',
      majority: Math.ceil(players.filter((player) => !player.isDead || isExile).length / 2),
      votes: players.filter((player, index) => state.votes[index]).map(({ name }) => name),
    })
  },
